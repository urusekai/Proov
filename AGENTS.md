...
- 비로그인: 공개 문제 세트 열람 및 풀이만 가능, GitHub 연동 없음
- PR URL 기반 문제 생성은 로그인 필수. 비로그인 접근 시 `/auth/login?redirect=...`으로 리다이렉트
- 로그인은 문제 생성/결과 저장/기록 조회 시 필요
...
- 별도 요청이 없으면 작업 후 브라우저로 화면을 확인하지 않는다.
