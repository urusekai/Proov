...
- 문제 세트 목록에서는 PR 제목을 대표 제목으로 쓰지 않는다. 별도 학습 제목, 요약, 태그, 난이도, 소요 시간을 먼저 보여주고 PR 제목/URL/pull number는 풀이 전 노출하지 않는다.
...
- Problem Set List: 학습 제목, 요약, 언어/프레임워크/라이브러리/주제 태그, 포함된 문제 유형 태그, 난이도, 소요 시간, repository, 최신순/난이도순/태그 필터. PR title/URL/pull number는 풀이 전 숨김.
